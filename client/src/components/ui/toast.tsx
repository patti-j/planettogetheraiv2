...
import { X, Clock, Clock3 } from "lucide-react"
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Viewport
    ref={ref}
    className={cn(
      "fixed bottom-4 right-4 z-[9990] flex max-h-screen w-full max-w-[420px] flex-col-reverse gap-2 p-4 pointer-events-none",
      className
    )}
    {...props}
  />
    ...
    >
      ...
      {onToggleAutoClose && (
        <button
          ...
          className={cn(
            "absolute right-14 top-2 rounded-md p-1.5 text-foreground/50 opacity-0 transition-opacity hover:text-foreground focus:opacity-100 focus:outline-none focus:ring-2 group-hover:opacity-100",
