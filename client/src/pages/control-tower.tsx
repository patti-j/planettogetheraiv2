...
function KpiTargetForm({ kpi, plants, onSave, isLoading }: {
  ...
  const [formData, setFormData] = useState({
    ...
    excellentThreshold: kpi?.excellentThreshold || 0.95,
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div>
          <Label htmlFor="excellentThreshold">Excellent Threshold</Label>
          <Input
            type="number"
            step="0.01"
            min="0"
            ...
            onChange={(e) => setFormData({ ...formData, excellentThreshold: parseFloat(e.target.value) })}
          />
        </div>
  ...
  return (
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid md:grid-cols-2 gap-4">
        <div>
          <Label htmlFor="plantId">Plant</Label>
          <Select value={formData.plantId.toString()} onValueChange={(value) => setFormData({...formData, plantId: value})}>
            <SelectTrigger>
              <SelectValue placeholder="Select plant" />
            </SelectTrigger>
            <SelectContent>
        ...
        <div>
          ...
          <Input
            ...
            value={formData.kpiName}
            onChange={(e) => setFormData({...formData, kpiName: e.target.value})}
            placeholder="e.g., OEE, Throughput, Quality"
            required
          />
        </div>
        ...
        <div>
          ...
          <Input
            ...
            value={formData.unitOfMeasure}
            onChange={(e) => setFormData({...formData, unitOfMeasure: e.target.value})}
            placeholder="e.g., %, units/hr, $, minutes"
            required
          />
        </div>
      ...
      <div>
        ...
        <Textarea
          ...
          value={formData.description}
          onChange={(e) => setFormData({...formData, description: e.target.value})}
          placeholder="Description of this KPI and how it's measured"
        />
      </div>

      <div className="flex justify-end space-x-2">
        <Button type="submit" disabled={isLoading}>
          {isLoading ? "Saving..." : "Save KPI Target"}
        </Button>
      </div>
    </form>
  );
...
function OptimizationConfigForm({ 
  optimization, 
  plants, 
  kpiTargets, 
  onSave, 
  ...
  const [formData, setFormData] = useState({
    name: optimization?.name || '',
    description: optimization?.description || '',
    plantId: optimization?.plantId || '',
    optimizationObjective: optimization?.optimizationObjective || 'weighted_kpi',
    targetKpiIds: optimization?.targetKpiIds || [],
    allowedAlgorithms: optimization?.allowedAlgorithms || ['ASAP', 'ALAP', 'CRITICAL_PATH', 'LEVEL_RESOURCES', 'DRUM_TOC'],
    currentAlgorithm: optimization?.currentAlgorithm || 'ASAP',
    autoAlgorithmSelection: optimization?.autoAlgorithmSelection ?? true,
    enableParameterTuning: optimization?.enableParameterTuning ?? true,
    learningMode: optimization?.learningMode || 'active',
    performanceThreshold: optimization?.performanceThreshold || 0.90,
    evaluationPeriodMinutes: optimization?.evaluationPeriodMinutes || 60,
    requiredApproval: optimization?.requiredApproval ?? true,
  });

  const availableKpis = kpiTargets.filter((kpi: PlantKpiTarget) => 
    !formData.plantId || kpi.plantId === parseInt(formData.plantId.toString())
  );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSave({
      ...
      performanceThreshold: parseFloat(formData.performanceThreshold.toString()),
      evaluationPeriodMinutes: parseInt(formData.evaluationPeriodMinutes.toString()),
    });
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="grid md:grid-cols-2 gap-4">
        <div>
          ...
          <Input
            ...
            value={formData.name}
            onChange={(e) => setFormData({...formData, name: e.target.value})}
            placeholder="e.g., Plant A Auto-Optimization"
            required
          />
        </div>
        ...
        <div>
          <Label htmlFor="plantId">Plant</Label>
          <Select value={formData.plantId.toString()} onValueChange={(value) => setFormData({...formData, plantId: value, targetKpiIds: []})}>
            <SelectTrigger>
              <SelectValue placeholder="Select plant" />
            </SelectTrigger>
            <SelectContent>
      ...
      <div>
        ...
        <Textarea
          ...
          value={formData.description}
          onChange={(e) => setFormData({...formData, description: e.target.value})}
          placeholder="Description of this optimization configuration"
        />
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div>
          ...
          <Select value={formData.optimizationObjective} onValueChange={(value) => setFormData({...formData, optimizationObjective: value})}>
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="weighted_kpi">Weighted KPI Score</SelectItem>
              <SelectItem value="single_kpi">Single KPI Focus</SelectItem>
        ...
        <div>
          <Label htmlFor="currentAlgorithm">Current Algorithm</Label>
          <Select value={formData.currentAlgorithm} onValueChange={(value) => setFormData({...formData, currentAlgorithm: value})}>
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="ASAP">ASAP (Forward Scheduling)</SelectItem>
              <SelectItem value="ALAP">ALAP (Backward Scheduling)</SelectItem>
              <SelectItem value="CRITICAL_PATH">Critical Path Method</SelectItem>
              <SelectItem value="LEVEL_RESOURCES">Level Resources</SelectItem>
              <SelectItem value="DRUM_TOC">Drum (Theory of Constraints)</SelectItem>
            </SelectContent>
          </Select>
        </div>
      </div>
      ...
      <div>
        <Label>Target KPIs for Optimization</Label>
        <div className="mt-2 space-y-2 max-h-32 overflow-y-auto">
          {availableKpis.map((kpi: PlantKpiTarget) => (
            <div key={kpi.id} className="flex items-center space-x-2">
              <input
                type="checkbox"
                id={`kpi-${kpi.id}`}
                ...
                onChange={(e) => {
                  if (e.target.checked) {
                    setFormData({...formData, targetKpiIds: [...formData.targetKpiIds, kpi.id]});
                  } else {
                    setFormData({...formData, targetKpiIds: formData.targetKpiIds.filter(id => id !== kpi.id)});
                  }
                }}
                className="rounded"
              />
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div>
          ...
          <Input
            ...
            max="1"
            value={formData.performanceThreshold}
            onChange={(e) => setFormData({...formData, performanceThreshold: e.target.value})}
            required
          />
          <p className="text-xs text-gray-600 mt-1">Trigger optimization when performance falls below this (0-1)</p>
        </div>

        <div>
          <Label htmlFor="evaluationPeriodMinutes">Evaluation Period (minutes)</Label>
          <Input
            id="evaluationPeriodMinutes"
            type="number"
            min="5"
            value={formData.evaluationPeriodMinutes}
            onChange={(e) => setFormData({...formData, evaluationPeriodMinutes: e.target.value})}
            required
          />
        </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center space-x-2">
          <Switch
            ...
            onCheckedChange={(checked) => setFormData({...formData, autoAlgorithmSelection: checked})}
          />
          <Label htmlFor="autoAlgorithmSelection">Auto Algorithm Selection</Label>
        </div>
        ...
        <div className="flex items-center space-x-2">
          <Switch
            ...
            onCheckedChange={(checked) => setFormData({...formData, enableParameterTuning: checked})}
          />
          <Label htmlFor="enableParameterTuning">Enable Parameter Tuning</Label>
        </div>
        ...
        <div className="flex items-center space-x-2">
          <Switch
            ...
            onCheckedChange={(checked) => setFormData({...formData, requiredApproval: checked})}
          />
          <Label htmlFor="requiredApproval">Require Approval for Changes</Label>
        </div>
      </div>
