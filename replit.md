...
-   **Production Scheduling**: Visual Gantt chart, operation sequencer, advanced scheduling algorithms, and constraints management. Includes a PT Resource Capabilities System for appropriate resource-operation matching using `ptresourcecapabilities`. Category column removed from resource grid for cleaner interface. Invalid drag operations show only tooltip message without duplicate error lines. Scheduling agent floating button removed for streamlined interface. Auto zoom-to-fit on initial load and view preset changes (Hour & Day, Day & Week, Week & Month, Month & Year), with manual zoom-to-fit button available in toolbar.
