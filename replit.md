...
### December 11, 2025
- **GPT-5.1 Token Configuration**: Fixed empty AI response issue by increasing `max_completion_tokens` to 4096 (from 800) to accommodate adaptive reasoning model's internal token usage
- **Stop Button for AI**: Implemented cancellable AI responses - send button transforms to red stop button during processing, allows request cancellation via AbortController
- **Ad-Hoc Report Display Fix**: Reports now render as visual tables on Canvas page instead of text-only chat responses:
  - Fixed agent registration order in `server/services/agents/agent-registry.ts` - Ad-Hoc Reporting Agent now registered first to handle report requests properly
  - Added `action` passthrough in `server/ai-agent.ts` for `open_report` actions
  - Added frontend handling in `client/src/components/navigation/ai-left-panel.tsx` to create table widgets from `open_report` actions
  - Fixed SQL query in `server/services/agents/adhoc-reporting-agent.service.ts` - changed `j.status` to `j.scheduled_status` (correct column name in ptjobs table)
- **Knowledge Base System**: Fresh HubSpot import with 184 articles and 1,035 searchable chunks working with GPT-5.1

### Technical Notes
- **GPT-5.1**: Uses `max_completion_tokens` parameter (not `max_tokens`). Adaptive reasoning consumes tokens before output. `finish_reason: "length"` indicates token exhaustion.
- **Agent Registration Order**: Ad-Hoc Reporting Agent must be registered before Production Scheduling Agent to handle report requests with proper `open_report` actions
- **Report Action Flow**: Backend returns `action: { type: 'open_report', data: {...} }` → Frontend handles in handleSendMessage → Creates table widget → Saves to canvas → Navigates to Canvas page
