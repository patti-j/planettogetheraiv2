...
- **Fixed Recent Pages Icon Issues** (Aug 13, 2025): Completely resolved recent pages showing generic FileText icons instead of menu-specific icons. Enhanced pageMapping with missing routes (demand-planning, master-data, scheduling-optimizer). Fixed replaceOldRoutes function to detect and replace FileText fallback icons with proper menu icons (Brain, Monitor, Package, Database, etc.). Added automatic icon repair system that works on page load.
