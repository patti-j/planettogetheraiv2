...
-   **Production Scheduling**: Visual Gantt chart, operation sequencer, advanced scheduling algorithms, and constraints management. Includes a PT Resource Capabilities System for appropriate resource-operation matching using `ptresourcecapabilities`. Category column removed from resource grid for cleaner interface. Invalid drag operations show only tooltip message without duplicate error lines. Scheduling agent floating button removed for streamlined interface.
